...
class GridWorld:
    def __init__(self, rows, cols, walls=[], pits=[], goals=[], live_reward=0.0, esc=[], escf=[], serp=[],serpf=[]):
        self.rows = rows
        self.cols = cols
        self.walls = walls
        self.goals = goals
...
#         return state_values, state_q_values
    ...
    def step(self, state, action, random=False):
        if(state in self.goals):
            return "Terminal Diamante", +10.0, None, True
        elif(state in self.pits):
            return "Terminal Bomba", 0.0, None, True
        else:
    ...
    def solve_value_iteration(self, gamma=1.0, horizon=10, init_state=(2,0)):
        ...
        for i in range(horizon):
            self.r_draw_background()
            self.r_draw_values(show_action=True)
...
#                 self.state_q_values = state_q_values_1
    ...
    def follow_policy(self, init_state=(-1,-1), flag_q=False, gamma=1.0):
        ...
        while True:
            ...
            if done:
                break
            if state in self.esc:
                for st in self.esc:
                    if state==st:
                        state=self.escf[self.esc.index(st)]
            if state in self.serp:
                for st in self.serp:
                    if state==st:
                        state=self.serpf[self.serp.index(st)]
            for i in range(dado):
                action = self.policy[state]
                sta=state
                state, reward, _, done = self.step(state, action, random=False)
                if done:
